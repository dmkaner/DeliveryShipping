...
var ObjectToStructConverter = /** @class */ (function () {
    /**
    ...
    return ObjectToStructConverter;
}());
exports.ObjectToStructConverter = ObjectToStructConverter;
var GrpcService = /** @class */ (function (_super) {
    __extends(GrpcService, _super);
    /**
    ...
    function GrpcService(config, options) {
        ...
        _this.activeServiceMap_ = new Map();
        _this.protos = {};
        ...
        if (config.customEndpoint) {
            _this.grpcCredentials = grpc.credentials.createInsecure();
        }
        _this.grpcMetadata = new grpc.Metadata();
        _this.grpcMetadata.add('x-goog-api-client', [
        ...
        if (config.grpcMetadata) {
            for (var prop in config.grpcMetadata) {
                if (config.grpcMetadata.hasOwnProperty(prop)) {
                    _this.grpcMetadata.add(prop, config.grpcMetadata[prop]);
                }
            }
        }
        _this.maxRetries = options.maxRetries;
        _this.userAgent = common_1.util.getUserAgentFromPackageJson(config.packageJson);
        _this.activeServiceMap_ = new Map();
        _this.protos = {};
        ...
        Object.keys(protoServices).forEach(function (name) {
            ...
            var services = _this.loadProtoFile(protoConfig.path, config);
            var serviceKey = ['google', protoConfig.service, name].filter(function (x) { return x; }).join('.');
            var service = services[serviceKey];
            _this.protos[name] = service;
    ...
    GrpcService.prototype.request = function (protoOpts, reqOpts, callback) {
        ...
        if (!this.grpcCredentials) {
            ...
            this.getGrpcCredentials_(function (err, credentials) {
                ...
                _this.grpcCredentials = credentials;
                _this.request(protoOpts, reqOpts, callback);
            });
            return;
        }
        ...
        var retryOpts = extend({
            ...
            request: function (_, onResponse) {
                respError = null;
                return service[protoOpts.method](reqOpts, metadata, grpcOpts, function (err, resp) {
                    if (err) {
    ...
    GrpcService.prototype.getGrpcCredentials_ = function (callback) {
        var _this = this;
        this.authClient.getClient().then(function (client) {
            var credentials = grpc.credentials.combineChannelCredentials(grpc.credentials.createSsl(), grpc.credentials.createFromGoogleCredential(client));
            if (!_this.projectId || _this.projectId === '{{projectId}}') {
                _this.projectId = client.projectId;
            }
     ...
     */
    GrpcService.prototype.loadProtoFile = function (protoPath, config) {
        var protoObjectCacheKey = [config.protosDir, protoPath].join('$');
        if (!GrpcService.protoObjectCache[protoObjectCacheKey]) {
            var services = proto_loader_1.loadSync(protoPath, {
                keepCase: false,
                defaults: true,
                bytes: String,
                longs: String,
    ...
    GrpcService.prototype.getService_ = function (protoOpts) {
        ...
        if (!service) {
            service = new proto[protoOpts.service](proto.baseUrl || this.baseUrl, this.grpcCredentials, extend({
